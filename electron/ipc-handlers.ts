...
export function registerIPCHandlers() {
    ...
    ipcMain.handle('db:get-project', async (_event, id: number) => {
        return dbClient.getProjectById(id);
    });

    ipcMain.handle('db:create-project', async (_event, data) => {
    ...
    ipcMain.handle('db:update-project', async (_event, id: number, data) => {
    ...
    ipcMain.handle('db:delete-project', async (_event, id: number) => {
        return dbClient.deleteProject(id);
    });

    ipcMain.handle('db:get-project-stats', async (_event, projectId: number) => {
    ...
    ipcMain.handle('db:get-articles', async (_event, projectId: number) => {
        return dbClient.getArticlesByProject(projectId);
    });

    ipcMain.handle('db:get-article', async (_event, id: number) => {
        return dbClient.getArticleById(id);
    });

    ipcMain.handle('db:get-articles-with-decisions', async (_event, projectId: number) => {
        return dbClient.getArticlesWithDecisions(projectId);
    });

    ipcMain.handle('db:search-articles', async (_event, projectId: number, query: string) => {
    ...
    ipcMain.handle('db:save-screening-decision', async (_event, decision) => {
    ...
    ipcMain.handle('db:update-screening-decision', async (_event, articleId: number, data) => {
        return dbClient.saveScreeningDecision({
    ...
    ipcMain.handle('db:get-screening-decision', async (_event, articleId: number) => {
        const decision = dbClient.getScreeningDecision(articleId) as any;
    ...
    ipcMain.handle('db:save-extracted-data', async (_event, data) => {
    ...
    ipcMain.handle('db:update-extracted-data', async (_event, articleId: number, data) => {
        return dbClient.saveExtractedData({
            article_id: articleId,
            ...data,
            population: data.population ? JSON.stringify(data.population) : undefined,
    ...
    ipcMain.handle('db:get-extracted-data', async (_event, articleId: number) => {
    ...
    ipcMain.handle('db:get-all-extracted-data', async (_event, projectId: number) => {
    ...
    ipcMain.handle('ai:screen-article', async (event, request) => {
        const article = dbClient.getArticleById(request.article_id) as any;
        if (!article) {
            throw new Error(`Article ${request.article_id} not found`);
        }

        const council = AICouncil.getInstance();
    ...
    ipcMain.handle('ai:screen-batch', async (event, requests) => {
        const results = [];
        for (const request of requests) {
            try {
                const result = await ipcMain.handleOnce('ai:screen-article', request);
                results.push(result);
    ...
    ipcMain.handle('ai:extract-data', async (_event, request) => {
        const article = dbClient.getArticleById(request.article_id) as any;
        if (!article) {
            throw new Error(`Article ${request.article_id} not found`);
        }
    ...
    ipcMain.handle('ai:get-usage-summary', async (_event, month: string) => {
    ...
    ipcMain.handle('ollama:download-model', async (event, request) => {
        const ollama = OllamaManager.getInstance();
    ...
    ipcMain.handle('ollama:cancel-download', async (_event, modelName: string) => {
        const ollama = OllamaManager.getInstance();
    ...
    ipcMain.handle('fs:select-pdf', async () => {
    ...
    ipcMain.handle('fs:upload-pdf', async (event, request) => {
        const { project_id, file_path } = request;

        // Create storage directory
        const storageDir = path.join(app.getPath('userData'), 'pdfs', String(project_id));
    ...
    ipcMain.handle('fs:export-csv', async (_event, request) => {
    ...
    ipcMain.handle('fs:export-prisma', async (_event, request) => {
        const stats = dbClient.getProjectStats(request.project_id) as any;
    ...
    ipcMain.handle('fs:open-data-folder', async () => {
    ...
    ipcMain.handle('settings:get-api-key', async () => {
        const setting = dbClient.getSetting('cloud_api_key_encrypted') as any;
    ...
    ipcMain.handle('export:prisma-diagram', async (_event, request: { project_id: number; output_path: string }) => {
        const db = dbClient.getDB();
        await exportManager.generatePRISMADiagram(db, request.project_id, request.output_path);
        return { success: true, path: request.output_path };
    });

    ipcMain.handle('export:revman-csv', async (_event, request: { project_id: number; output_path: string }) => {
        const db = dbClient.getDB();
        exportManager.exportToRevManCSV(db, request.project_id, request.output_path);
        return { success: true, path: request.output_path };
    });

    ipcMain.handle('export:revman-xml', async (_event, request: { project_id: number; output_path: string }) => {
        const db = dbClient.getDB();
        exportManager.exportToRevManXML(db, request.project_id, request.output_path);
        return { success: true, path: request.output_path };
    });

    ipcMain.handle('export:json', async (_event, request: { project_id: number; output_path: string }) => {
        const db = dbClient.getDB();
        await exportManager.exportToJSON(db, request.project_id, request.output_path);
        return { success: true, path: request.output_path };
    });

    ipcMain.handle('export:all', async (_event, request: { project_id: number; output_dir: string }) => {
        const db = dbClient.getDB();
        const paths = await exportManager.exportAll(db, request.project_id, request.output_dir);
        return { success: true, paths };
    });

    ipcMain.handle('fs:select-export-location', async () => {
        const result = await dialog.showOpenDialog({
            properties: ['openDirectory', 'createDirectory'],
            title: 'Select Export Directory'
        });

        return { directory: result.filePaths[0] || null };
    });
}
...
function parseExtractedDataJSON(data: any) {
    return {
        ...data,
...
async function extractPDFInBackground(articleId: number, pdfPath: string, event: any) {
    try {
...
function generateRevManCSV(data: any[], format: string): string {
...
function generatePRISMASVG(stats: any): string {
